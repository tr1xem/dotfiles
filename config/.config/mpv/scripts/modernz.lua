...
local function build_icons(theme_name, style)
	...
	local function f(name)
		return p .. name .. filled_suffix
	end
	local function o(name)
		return p .. name .. outline_suffix
	end
...
local state = {
	...
	last_touchX = -1,
	last_touchY = -1, -- last touch position
...
local function window_controls()
	...
	if user_opts.window_controls then
		local size_hover = contains(user_opts.hover_effect, "size")
				and string.format("\\fscx%s\\fscy%s", user_opts.hover_button_size, user_opts.hover_button_size)
			or ""
...
layouts["modern"] = function()
	...
	setup_bg_elements(
		...
		posY,
		osc_w,
		user_opts.fade_transparency_strength,
	...
	local show_remhours = (state.tc_left_rem and remsec >= 3600)
		or (not state.tc_left_rem and dur >= 3600)
...
layouts["modern-compact"] = function()
	...
	local osc_height_offset = (
		...
		+ (
			(
					(
						user_opts.chapter_title_mbtn_left_command == ""
						and user_opts.chapter_title_mbtn_right_command == ""
					) or not chapter_index
				)
				and 10
			or 0
		)
	...
	lo.geometry = geo
	lo.style =
		string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.title, geo.x, geo.y - geo.h, geo.x + geo.w, geo.y + geo.h)
	lo.alpha[3] = 0
	...
	if user_opts.show_chapter_title then
		...
		lo.style = string.format(
			...
			osc_styles.chapter_title,
			chapter_geo.x,
			chapter_geo.y - chapter_geo.h,
			chapter_geo.x + chapter_geo.w,
			chapter_geo.y + chapter_geo.h
		)
	end
	-- Time codes
	...
	lo = add_layout("quality-menu")
	lo.geometry = { x = end_x, y = refY - 35, an = 5, w = 24, h = 24 }
	lo.style = osc_styles.osc_styles.control_2
	end_x = end_x - 55
	-- END quality-menu
	...
	if elements.cache_info.visible then
		lo = add_layout("cache_info")
		lo.geometry = { x = end_x + 11, y = refY - 35, an = 6, w = (user_opts.cache_info_speed and 70 or 45), h = 24 }
		lo.style = osc_styles.time
	end
end
...
local function osc_visible(visible)
	set_bar_visible("osc_visible", visible, true, function()
...
local function osc_init()
	...
	-- playlist buttons
	local pl_nav_visible_w = (state.is_image and 300 or 500)
		- nojumpoffset
		- noskipoffset * (nojumpoffset == 0 and 1 or 10)
	-- prev
	...
	ne.eventresponder["mbtn_right_down"] = function()
		if user_opts.loop_in_pause then
			mp.command(
				"show-text '" .. (state.file_loop and locale.file_loop_disable or locale.file_loop_enable) .. "'"
			)
	...
	ne.eventresponder["shift+mbtn_left_down"] = function()
		if user_opts.loop_in_pause then
			mp.command(
				"show-text '"
					.. (state.playlist_loop and locale.playlist_loop_disable or locale.playlist_loop_enable)
					.. "'"
			)
	...
	ne.tooltipF = user_opts.tooltip_hints
			and (have_pl and locale.playlist .. " [" .. pl_pos .. "/" .. pl_count .. "]" or locale.playlist .. " / " .. locale.menu)
		or ""
	...
	ne.tooltipF = function()
		local prop = mp.get_property("current-tracks/audio/title")
			...
			or locale.na
		return (
			user_opts.tooltip_hints
				and (locale.audio .. " " .. mp.get_property_number("aid", "-") .. "/" .. state.audio_track_count .. " [" .. prop .. "]")
			or ""
		)
	end
	...
	ne.tooltipF = function()
		...
		return (
			user_opts.tooltip_hints
				and (locale.subtitle .. " " .. mp.get_property_number("sid", "-") .. "/" .. state.sub_track_count .. " [" .. prop .. "]")
			or ""
		)
	end
	...
	ne.content = function()
		return state.file_loop and icons.loop_on or icons.loop_off
	end
	ne.visible = (osc_param.playresx >= visible_min_width)
	ne.tooltip_style = osc_styles.tooltip
	ne.tooltipF = function()
		return user_opts.tooltip_hints and (state.file_loop and locale.file_loop_disable or locale.file_loop_enable)
			or ""
	end
	ne.eventresponder["mbtn_left_up"] = function()
	...
	ne.content = function()
		return state.shuffled and icons.shuffle_on or icons.shuffle_off
	end
	ne.visible = (osc_param.playresx >= visible_min_width)
	ne.tooltip_style = osc_styles.tooltip
	ne.tooltipF = function()
		return user_opts.tooltip_hints and (state.shuffled and locale.unshuffle or locale.shuffle) or ""
	end
	ne.eventresponder["mbtn_left_up"] = function()
	...
	ne.eventresponder["mbtn_left_up"] = function()
		adjust_speed(user_opts.speed_button_click)
	end
	ne.eventresponder["mbtn_right_up"] = function()
		mp.commandv("set", "speed", 1)
	end
	ne.eventresponder["wheel_up_press"] = function()
		adjust_speed(user_opts.speed_button_scroll)
	end
	ne.eventresponder["wheel_down_press"] = function()
		adjust_speed(-user_opts.speed_button_scroll)
	end
	...
	ne.tooltipF = function()
		...
		local abitrate = mp.get_property_number("audio-bitrate", 0)

		local ainfo = acodec
		if abitrate > 0 then
			ainfo = string.format("%s %dk", acodec, math.floor(abitrate / 1000))
		end
		...
		local fps_str = ""
		if fps > 1 then
			fps_str = tostring(math.floor(fps + 0.5))
		end

		local vinfo = string.format("%dp%s (%s)", h, fps_str, vcodec)

		return "Video: " .. vinfo .. " Audio: " .. ainfo
	end

	ne.eventresponder["mbtn_left_up"] = function()
		mp.commandv("script-message", "video_formats_toggle")
	end
	ne.eventresponder["mbtn_right_up"] = function()
		mp.commandv("script-message", "audio_formats_toggle")
	end
	-- END quality-menu
	...
	ne.eventresponder["mbtn_left_up"] = function(element)
		...
		if state.playing_and_seeking then
			...
			if
				not element.state.was_paused
				and mp.get_property("eof-reached") == "no"
				and user_opts.mouse_seek_pause
			then
				mp.commandv("cycle", "pause")
			end
			state.playing_and_seeking = false
	...
	ne.content = function()
		...
		local playtime_remaining = state.tc_left_rem and mp.get_property_number("playtime-remaining", 0)
			or playback_time
	...
	elseif source == "mouse_move" then
		...
		then
			...
			if user_opts.bottomhover then -- if enabled, only show osc if mouse is hovering at the bottom of the screen (where the UI elements are)
				local top_hover = window_controls_enabled()
					and (user_opts.show_window_title or user_opts.window_controls)
...
local function render()
	...
	elseif state.initREQ then
		...
		if
			(state.last_mouseX == nil or state.last_mouseY == nil)
			and not (mouseX == nil or mouseY == nil or mouseX == -1 or mouseY == -1)
		then
			state.last_mouseX, state.last_mouseY = mouseX, mouseY
		end
	end
	...
	local function run_fade(anitype_key, anistart_key, animation_key, set_visible)
		...
		if now < state[anistart_key] + fadelen then
			if anitype == "in" then
				set_visible(true)
				state[animation_key] = scale_value(state[anistart_key], state[anistart_key] + fadelen, 255, 0, now)
			elseif anitype == "out" then
				state[animation_key] = scale_value(state[anistart_key], state[anistart_key] + fadelen, 0, 255, now)
			end
		else
			if anitype == "out" then
				set_visible(false)
	...
	update_input_area("input", state.osc_visible, "input_enabled", function()
		mp.enable_key_bindings("input")
	end)
	update_input_area("window-controls", wc_vis, "windowcontrols_buttons", function()
		mp.enable_key_bindings("window-controls")
	end)
	update_input_area("window-controls-title", wc_vis, "windowcontrols_title", function()
	...
	local function run_autohide(showtime_key, hide_fn, input_areas)
		if state[showtime_key] == nil or get_hidetimeout() < 0 then
			return
		end
		-- keeponpause + independent_zones: bottom bar is locked, top bar hides normally
		if state.pause_osc_locked and showtime_key == "showtime" then
			return
		end
		local timeout = state[showtime_key] + (get_hidetimeout() / 1000) - now
		if timeout <= 0 and get_touchtimeout() <= 0 then
			if
				(state.active_element == nil and not mouse_in_area(input_areas))
				or (state.active_element == nil and not user_opts.osc_keep_with_cursor)
			then
				hide_fn()
			end
		else
	...
	if state.hide_timer then
		state.hide_timer.timeout = math.huge
	end
...
tick = function()
	...
	local function tick_animation(anitype_key, anistart_key, animation_key)
		if state[anitype_key] ~= nil then
			if
				not state.idle
				and (not state[anistart_key] or mp.get_time() < 1 + state[anistart_key] + user_opts.fadeduration / 1000)
			then
				request_tick()
			else
				kill_animation(anitype_key, anistart_key, animation_key)
...
mp.register_event("file-loaded", function()
	...
	if oos == true or oos == "yes" then
		oos = "both"
	end
	if oos == "bottom" or oos == "both" then
		show_osc()
	end
	if oos == "top" or oos == "both" then
		show_wc()
	end
end)
